...
SERVER_SCRIPT="$REPO_ROOT/scripts/server.mjs"
...
case "$CMD" in
  ...
  status)
    ...
    if launchctl print "gui/$(id -u)/com.nova.tracker" >/dev/null 2>&1; then
      echo "[Nova Service] Status: RUNNING (registered in GUI domain)"
      launchctl print "gui/$(id -u)/com.nova.tracker" | grep -E "pid =" || true
    else
